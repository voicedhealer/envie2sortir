...
export default function AllEstablishments() {
  ...
  useEffect(() => {
    loadResults(activeFilter, 1, true);
  }, []);

  // Charger plus de résultats
  const loadMore = () => {
    if (!loadingMore && pagination.hasMore) {
      loadResults(activeFilter, pagination.currentPage + 1, false);
    }
  };
  ...
  const handleFilterChange = (filter: string) => {
    ...
    loadResults(filter, 1, true);
  };

  // Fonction pour charger les résultats
  const loadResults = async (filter: string, page: number, reset: boolean) => {
    try {
      if (reset) {
        setLoading(true);
        setError(null);
      ...
      const response = await fetch(`/api/establishments/all?${params}`);
